...
/**
 * Adds a loot pool to an entity table that yields a specific item with a given probability.
 * @param {LootTableEventJS} event
 * @param {string} entityId namespaced ID (e.g., minecraft:witch)
...
const addEntityDropWithChance = (event, entityId, itemId, chance, options) => {
	...
	if (chance <= 0 || chance > 1) {
		throw `Chance ${chance} for ${entityId} -> ${itemId} is outside (0,1]`
	}
	const tableId = normalizeEntityTableId(entityId)
	const table = ensureTable(event, tableId)
	if (!table) {
		throw `No loot table found for ${entityId}`
	}
	...
	const fallbackName = entityId.replace(":", "_") + "_" + itemId.replace(":", "_")
